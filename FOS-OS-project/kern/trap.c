...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	int replaced=0;
	uint32 entryindex = 0;
	uint32 *pointerpagetable;
	uint32 currentpagevirtualaddress;
	uint32 permissions;
	unsigned int maximumsize= curenv->page_WS_max_size;
	int flag=0;
	uint32 currentsize= env_page_ws_get_size(curenv);

	if (currentsize<maximumsize){
         //placement
		entryindex=curenv->page_last_WS_index;
		flag=1;
		goto placement;
	}
	else{
		...
		for(unsigned int i=0;i<maximumsize;i++){
		    ...
		    currentpagevirtualaddress = env_page_ws_get_virtual_address(curenv,index);
			permissions=pt_get_page_permissions(curenv,currentpagevirtualaddress);
			if(((permissions&PERM_USED)==0) && ((permissions&PERM_MODIFIED)==0)){
				clockreturn=1;
				entryindex=index;
                env_page_ws_clear_entry(curenv,entryindex);
				break;
			}
			index++;
		}
		Try2:
		if(clockreturn==0){
			uint32 index1=curenv->page_last_WS_index;
			for(unsigned int i=0;i<currentsize;i++){
				if(index1==maximumsize){
					index1=0;
				}
				currentpagevirtualaddress = env_page_ws_get_virtual_address(curenv,index1);
				permissions=pt_get_page_permissions(curenv,currentpagevirtualaddress);
				// if USED == 1
				if(permissions&PERM_USED){
					pt_set_page_permissions(curenv,currentpagevirtualaddress,0,PERM_USED);
				}
				else{
 				// if USED == 0
                    replaced=1;
		...
		if(replaced==0 && clockreturn==0){
			goto Try1;
		}
		else{
			if(permissions&PERM_MODIFIED){
				struct Frame_Info *frame2;
				frame2=get_frame_info(curenv->env_page_directory,(void *)currentpagevirtualaddress,&pointerpagetable);
				pf_update_env_page(curenv,(void *)currentpagevirtualaddress,frame2);
				unmap_frame(curenv->env_page_directory,(void *)currentpagevirtualaddress);
			}
			else{
				unmap_frame(curenv->env_page_directory,(void *)currentpagevirtualaddress);
			}
	...
	//clear entry
	//map
	frame=get_frame_info(curenv->env_page_directory,(void *)currentpagevirtualaddress,&pointerpagetable);
	allocate_frame(&frame);
	map_frame(curenv->env_page_directory,frame,(void *)fault_va,PERM_USER|PERM_WRITEABLE|PERM_MODIFIED);
	int readvalue = pf_read_env_page(curenv,(void *)fault_va);
	if(readvalue==E_PAGE_NOT_EXIST_IN_PF){
		if(fault_va < USTACKTOP && fault_va >= USTACKBOTTOM){
			...
			panic("ERROR: Page not Exist in Page File");
		    }
	}
	//reflect changes in the page working set
	if(flag==1){
		uint32 lasttindex=curenv->page_last_WS_index;
		for(int i=curenv->page_WS_max_size;i>0;i--){
			lasttindex=lasttindex%curenv->page_WS_max_size;
			if(env_page_ws_is_entry_empty(curenv,lasttindex)){
				curenv->page_last_WS_index=(lasttindex+1) % curenv->page_WS_max_size;
				break;
			}
			lasttindex++;
		}
		env_page_ws_set_entry(curenv,lasttindex,ROUNDDOWN(fault_va,PAGE_SIZE));
	}
	else{
		curenv->page_last_WS_index=(entryindex+1) % curenv->page_WS_max_size;
		env_page_ws_set_entry(curenv,entryindex,ROUNDDOWN(fault_va,PAGE_SIZE));
	}
