...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	//set CPU quantum by the first level one
...
struct Env* fos_scheduler_MLFQ()
{
	...
	unsigned int currentenvstatus;
	struct Env* nextenv=NULL;
	struct Env* foundenv=NULL;
	int envfound=0;
	int index=0;
	//Steps:
	//======
	...
	if(curenv!=NULL){
		if(index2==(num_of_ready_queues-1)){
		    enqueue(&(env_ready_queues[index2]),curenv);
		}
		else{
			enqueue(&(env_ready_queues[index2+1]),curenv);
		}
	}
	...
	//	  Else, return NULL
	if(envfound==1){
		kclock_set_quantum(quantums[index]);
		nextenv=dequeue(&(env_ready_queues[index]));
		index2=index;
		return nextenv;
	}
	else{
