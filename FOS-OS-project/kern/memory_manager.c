...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	int b= 1024;
		uint32 phy;
		uint32 * pointer;
	    pointer = kmalloc(PAGE_SIZE);
	     phy = kheap_physical_address((unsigned int)pointer);
	     phy |= (PERM_PRESENT | PERM_WRITEABLE |PERM_USER);
	     ptr_page_directory[PDX(virtual_address)]=phy;
	     int z= 0;
	      do {
	        pointer[z] = 0;
	         z++;
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 size1=ROUNDUP(size,PAGE_SIZE)/PAGE_SIZE;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//panic("freeMem() is not implemented yet...!!");
		...
		get_page_table(e->env_page_directory,(void *)i,&pointerpagetable);
		x=0;
			if(pointerpagetable!=NULL){
				for(uint32 b=0;b<1024;b++){
				if(pointerpagetable[b]!=0){
					x=1;
					break;
				}
			}
